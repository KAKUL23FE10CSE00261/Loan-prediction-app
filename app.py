...
LoanAmount = st.sidebar.number_input("Loan Amount (₹ thousands)", min_value=0)
...
# FIX: feature list now matches input_data exactly (12 features)
FEATURE_NAMES = [
...
with col1:
    ...
    if LoanAmount > 0 and Loan_Amount_Term > 0:
        annual_rate = 0.085  # assumed 8.5% annual interest rate
...
with col2:
    ...
    if hasattr(model, "feature_importances_"):
        ...
        # FIX: use FEATURE_NAMES which matches the 12 features passed to the model
        fig, ax = plt.subplots()
